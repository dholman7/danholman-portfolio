"""
Python security standards ruleset.

This ruleset is generated from source markdown files and provides
organizational Python security standards.
"""
...
class SecurityRuleset:
    ...
    def __init__(self):
        ...
        source_file = self.sources_dir / "python" / "security.md"
    ...
    def apply(self, output_dir: str = ".cursor/rules", format: str = "cursor"):
        """Apply the security ruleset to the specified output directory."""
        output_path = Path(output_dir)
        
        if format == "cursor":
            renderer = CursorRenderer()
            filename = "python-security-standards.mdc"
        elif format == "copilot":
            renderer = CopilotRenderer()
            filename = "python-security-standards.instructions.md"
        else:
